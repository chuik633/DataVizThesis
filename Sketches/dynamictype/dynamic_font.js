...
//sound controls
function start() {sound.play(0, 1, 2, 90);}
function stop(){sound.stop()}

function drawText(inputText, x,y, maxWidth, animateNoise, animatePos, animateSize){
    ...
    for(let letterIdx = 0; letterIdx< inputText.length; letterIdx++){
        ...
        let amp_average = 0
        for(let i = sample_size*letterIdx; i<sample_size*(letterIdx+1) && i< spectrum.length; i++){
        ...
        sampleFac = map(letter_fontSize, 10, 100, .8, .3) //set font size accordingly
        let letter_points = font.textToPoints(letter,x,y - y_shift,letter_fontSize,{ sampleFactor:  sampleFac})
        
        //Draw letter from points
        if(animateNoise){
            drawLetterFromPoints(letter_points,amp_average, fontSize/8)
        }else{
...
function drawLetterFromPoints(points, amp, noiseSize){
    ...
    for(const point of points){ 
        let n_zoom = .1
        let n_speed = map(amp,0,255, .001, .3)
        let n =  noise(n_zoom*point.x,n_zoom* point.x, frameCount *n_speed)*noiseSize
